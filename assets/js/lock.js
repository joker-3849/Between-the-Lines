  ...
  body.querySelector('#passForm').addEventListener('submit', async e => {
    ...
    if (canPublish()) {
      try {
        ...
        // Токен не расшифровался текущей фразой: либо она набрана неверно,
        // либо club.json правили руками и фраза с токеном разошлись. Менять
        // фразу нельзя — сохранение после этого перестало бы работать.
        return fail('Токен сохранения не расшифровался текущей фразой, поэтому фраза '
          + 'не сменена. Проверьте, верно ли она набрана. Если фраза и токен разошлись '
          + 'совсем, выключите сохранение на вкладке «Сохранение» и включите заново.');
      }
    }
